...
> **Everything you need to know to use Z-Image Turbo Pro, step by step.**
...
**Z-Image Turbo Pro** is a tool that generates images using AI. You describe what you want in text, and the AI creates it.
...
Z-Image Turbo Pro uses FP8, which means you can run it on Google Colab's **free** T4 GPU without running out of memory.
...
1. **Click this link** (or the "Open in Colab" button in the README):
   ...
   👉 [Open ZImagePro in Google Colab](https://colab.research.google.com/github/Shineii86/ZImagePro/blob/main/notebook/ZImagePro.ipynb)
...
1. **Downloads the ZImagePro code** — the project files (this repository)
...
|-------|------|-------------|
| **Z-Image Turbo FP8** | ~4 GB | The main image generator — this is the core AI |
...
| `steps` | Slider | 20 | 10–50 | Number of denoising iterations. More steps = slightly more detail, but slower. 20 is the sweet spot for Z-Image Turbo. |
| `guidance_scale` | Slider | 1.0 | 1.0–10.0 | **Classifier-Free Guidance** — how strictly the AI follows your prompt. Lower = more creative freedom, higher = stricter adherence. 1.0 works well with Z-Image. |
...
| **Best quality** | `res_multistep` | `beta` | Optimized for Z-Image — slightly better detail |
...
| `res_multistep` | ⚡⚡⚡ | ⭐⭐⭐⭐⭐ | Best for Z-Image |
...
|------|-------|-------------|
| **Z-Image FP8 model** | `/content/ComfyUI/models/diffusion_models/` | 📁 File browser → ComfyUI → models → diffusion_models |
...
The ZImagePro project is MIT licensed. However, check the license of the specific Z-Image model you use — it may have its own terms.
</details>
...
20 steps provides excellent quality with the `euler` sampler. You can reduce to 10–15 for faster generation or increase to 30–50 for maximum detail. Z-Image Turbo converges quickly — going higher has diminishing returns.
</details>
...
**What it means:** The ZImagePro repository wasn't cloned properly.
...
**Made with ⚡ by [Shinei Nouzen](https://github.com/Shineii86)**

[![GitHub](https://img.shields.io/badge/-GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Shineii86/ZImagePro)
